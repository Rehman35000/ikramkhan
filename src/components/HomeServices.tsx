...
const services = [
  ...
  },
  {
    ...
    description: 'Cloud-native infrastructure designed for reliability and scale.',
    icon: (
      <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    <section className="py-24 px-6">
      <div className="max-w-6xl mx-auto">
        <motion.div
          ...
          className="mb-14"
        >
          <p className="text-xs font-semibold tracking-widest uppercase text-muted mb-3">Services</p>
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-3">
            What we <span className="gradient-text">build</span>
          </h2>
          <p className="text-muted-foreground max-w-lg">
            End-to-end technology solutions that drive growth.
          </p>
        </motion.div>
        ...
        >
          {services.map((service) => (
            <motion.div
              ...
              whileHover={{ y: -2 }}
              className="group p-6 rounded-xl border border-border bg-card hover:border-foreground/10 transition-all duration-200"
            >
              <div className="w-10 h-10 rounded-lg bg-surface flex items-center justify-center text-foreground mb-4">
                {service.icon}
              </div>
              ...
              <Link
                href="/services"
                className="inline-flex items-center text-xs font-medium text-muted hover:text-foreground transition-colors"
              >
                Learn more
